...
async def get_user_settings(from_user, data: str, uset_data: str):
    ...
    cookies_path = ospath.join('cookies', f'{user_id}.txt')
    user_dict = user_data.get(user_id, {})
    premium_left = status_user = daily_limit = ''
    image = None

    attachment_url = user_dict.get('attachment_url', '')
    attachment_text = f'<b>• Attachment URL:</b> <code>{attachment_url}</code>' if attachment_url else '<b>• Attachment URL:</b> Not Set'

    if not data:
        sendpm, buttonkey = (" ✓", "✓ Send PM")
        ...
        # Add stream link toggle
        show_stream, buttonkey = (' ✓', ' ✓ Stream Button') if user_dict.get('show_stream_link', True) else (' ✘', 'Stream Button')
        buttons.button_data(buttonkey, f'userset {user_id} show_stream_link {not user_dict.get("show_stream_link", True)}')

        # Add custom YTDLP cookies toggle/upload
        cookies_enabled = user_dict.get('ytdlp_cookies_enabled', False)
        cookies_exists = await aiopath.exists(cookies_path)
        if cookies_exists:
            cookies_status = ' ✓' if cookies_enabled else ' ✘'
            buttons.button_data(f'{cookies_status} YTDLP Cookies', f'userset {user_id} ytdlp_cookies_toggle {not cookies_enabled}')
            buttons.button_data('Change Cookies', f'userset {user_id} prepare ytdlp_cookies')
            buttons.button_data('Remove Cookies', f'userset {user_id} rem_ytdlp_cookies')
        else:
            buttons.button_data('Set YTDLP Cookies', f'userset {user_id} prepare ytdlp_cookies')

        # Add the Attachment URL button only in the main menu
        buttons.button_data('Set Attachment URL', f'userset {user_id} setdata attachment_url')
        ...
        if uset_data in {'thumb', 'rclone_config', 'token_pickle', 'ytdlp_cookies'}:
            file_dict = {'thumb': (thumbpath, 'Thumbnail', 'Send a photo to save it as custom thumbnail.', '', ''),
                         ...
                         'ytdlp_cookies': (cookies_path, 'Cookies', 'Send a valid *.txt file for <b>cookies.txt</b>.', config_dict['IMAGE_YT'], '')}
            ...
            uset_dict = {'excluded_extensions': ('excluded_extensions', 'Extension', UsetString.EXT, config_dict['IMAGE_EXTENSION'], ''),
                         ...
                         'attachment_url': ('attachment_url', 'Attachment URL', 'Send your Attachment URL to be used.', '', '')}
        ...
        if uset_data not in ['setcap', 'index_url', 'token_pickle', 'gdrive_id', 'rclone_path', 'rclone_config', 'ytdlp_cookies']:
            buttons.button_data('Back', f'userset {user_id} back')
        ...
        msg_cookies = 'Send new valid *.txt file to change current <b>cookies.txt</b>.\n\n<i>Timeout: 60s.</i>' if await aiopath.exists(cookies_path) else \
            'Send a valid *.txt file for <b>cookies.txt</b>.\n\n<i>Timeout: 60s.</i>'
        prepare_dict = {'thumb': (msg_thumb, image),
                        ...
                        'ytdlp_cookies': (msg_cookies, config_dict['IMAGE_YT']),
                        ...
                        'attachment_url': ('Send your Attachment URL to be used.', '')}
...
async def set_user_settings(_, message: Message, query: CallbackQuery, key: str):
    ...
    if (key == 'dump_ch' and value.isdigit() or value.startswith('-100')):
        ...
        if not value.startswith(('http://', 'https://')):
            ...
            return await editMessage('Invalid URL! Send a valid URL starting with http/https', query.message)
        # Store the attachment URL
        await update_user_ldata(user_id, 'attachment_url', value)
...
async def add_rclone_pickle(_, message: Message, query: CallbackQuery, key: str):
    user_id = message.from_user.id
    handler_dict[user_id] = False
    file_path, ext_file = ('rclone', '.conf') if key == 'rclone_config' else ('tokens', '.pickle') if key == 'token_pickle' else ('cookies', '.txt')
    ...
    if message.document.file_name.endswith(ext_file):
        ...
        qdata = 'rctool' if key == 'rclone_config' else 'gdtool' if key == 'token_pickle' else ''
...
async def add_ytdlp_cookies(_, message: Message, query: CallbackQuery):
    user_id = message.from_user.id
    handler_dict[user_id] = False
    cookies_dir = ospath.join(getcwd(), 'cookies')
    await makedirs(cookies_dir, exist_ok=True)
    if message.document.file_name.endswith('.txt'):
        des_dir = ospath.join(cookies_dir, f'{user_id}.txt')
        msg = await sendMessage('<i>Processing, please wait...</i>', message)
        await message.download(file_name=des_dir)
        await gather(update_user_ldata(user_id, 'ytdlp_cookies_enabled', True), deleteMessage(message, msg), update_user_settings(query))
        if DATABASE_URL:
            await DbManager().update_user_doc(user_id, 'ytdlp_cookies', des_dir)
    else:
        msg = await sendMessage('Invalid file! Please upload a .txt cookies file.', message)
        await gather(update_user_settings(query, 'setdata', 'ytdlp_cookies'), auto_delete_message(message, msg, stime=5))
...
async def edit_user_settings(client: Client, query: CallbackQuery):
    ...
    match data[2]:
        ...
        case 'rem_prename' | 'rem_sufname' | 'rem_dump_ch' | 'rem_remname' | 'rem_metadata' | 'rem_session_string' | 'rem_yt_opt' | 'rem_index_url' \
            | 'rem_gdrive_id' | 'rem_captions' | 'rem_excluded_extensions' | 'rem_rclone_path' | 'rem_ytdlp_cookies' | 'rem_attachment_url' as value:
            qdata = uset_data = ''
            match value:
                case 'rem_dump_ch':
        ...
        case 'enable_pm' | 'enable_ss' | 'as_doc' | 'media_group' | 'fnamecap' | 'stop_duplicate' | 'use_sa' | 'ytdlp_cookies_toggle' as value:
            qdata = uset_data = ''
            if value == 'ytdlp_cookies_toggle':
                await update_user_ldata(user_id, 'ytdlp_cookies_enabled', literal_eval(data[3]))
            else:
        ...
        case 'rem_thumb' | 'rem_rclone_config' | 'rem_token_pickle' | 'rem_ytdlp_cookies' as value:
            match value:
                case 'rem_thumb':
                    path = ospath.join('thumbnails', f'{user_id}.jpg')
                case 'rem_rclone_config':
                    path = ospath.join('rclone', f'{user_id}.conf')
                case 'rem_token_pickle':
                    path = ospath.join('tokens', f'{user_id}.pickle')
                case 'rem_ytdlp_cookies':
                    path = ospath.join('cookies', f'{user_id}.txt')
        ...
        case 'prepare':
            match data[3]:
                case 'rclone_config' | 'token_pickle' | 'ytdlp_cookies':
                    await query.answer()
                    photo, document = False, True
                    pfunc = partial(add_rclone_pickle, query=query, key=data[3]) if data[3] != 'ytdlp_cookies' else partial(add_ytdlp_cookies, query=query)
                case 'thumb':
                    await query.answer()
        ...
        case 'show_stream_link':
            await update_user_ldata(user_id, 'show_stream_link', literal_eval(data[3]))
            await gather(query.answer(), update_user_settings(query))
...
async def send_users_settings(client: Client, message: Message):
    ...
    for index, (uid, data) in enumerate(user_data.items(), start=1):
        ...
        for key, value in data.items():
            ...
            elif key in ['thumb', 'rclone_config', 'token_pickle', 'ytdlp_cookies']:
                value = 'Exists' if value else 'Not Exists'
            elif key in ['dump_ch', 'yt_opt', 'index_url', 'gdrive_id', 'prename', 'sufname', 'metadata', 'attachment_url']:
