...
class UseCheck:
    ...
    async def run(self, limit=False, forpremi=False, daily=False, ml_chek=False, session=False, send_pm=False):
        ...
        if session and (msg := await self._check_session(buttons)):
            msgs.append(msg)
        if send_pm and self._message.chat.type.name in ['SUPERGROUP', 'CHANNEL'] and (msg := await self._send_pm(buttons)):
            msgs.append(msg)
    ...
    async def _send_pm(self, buttons):
        try:
            ...
            if user.status.name == "LONG_AGO":
                ...
                return "⁍ User is inactive. Please start a private chat with the bot!"
        except:
            buttons.button_link("Start PM", f"http://t.me/{bot_name}")
            return "⁍ Unable to access private messages. Please start a private chat with the bot!"
