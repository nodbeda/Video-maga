...
from requests import Session, session as req_session, post as rpost, get
import requests
import sys
from requests.adapters import HTTPAdapter
from time import sleep
from urllib.parse import parse_qs, urlparse, quote, unquote, urljoin, urlunparse
...
class siteList:
    ...
    @property
    def all(self):
        return natsorted(self.DOOD + self.HOSTER + self.LIION_WISH + self.TERA + self.GSHARER + self.FEMBED + self.STAPE + self.RCL)
...
def direct_link_generator(link: str):
    ...
    if 'gofile.io' in domain:
        return gofile(link)
    if 'gdlink.dev' in domain:
        return gdflix(link)  # Handles gdlink.dev
    if 'new6.gdflix.dad' in domain:   
        return gdflix(link)  # Handles new6.gdflix.dad
    if 'vifix.site' in domain:
        return gdflix(link)  
    if 'new7.gdflix.dad' in domain:
        return gdflix(link)  
    if 'hubcloud.bz' in domain:
        return hubcloud(link)
    if 'hubcloud.ink' in domain:
        return hubcloud(link)
    if 'hubcloud.lol' in domain:
        return hubcloud(link)
    ...
    if 'u.pcloud.link' in domain:
        return pcloud(link)
    if any(x in domain for x in ["pixeldrain.com", "pixeldra.in", "pixeldrain.net"]):
    ...
    if '1tamilblasters.moi' in domain:
        return tamilblasters(link)
    if '1tamilblasters.tw' in domain:
        return tamilblasters(link)
...
def devuploads(url):
    ...
    proxy = "http://downloaderzone:downloaderzone@154.36.110.199:6853" #port 6600
...
def gdflix(link):
    """
    Fetches and groups downloadable links from a GDFlix page.
    Returns a Telegram-compatible HTML message.
    """

    # Normalize known domains
    if 'gdlink.dev' in link:
        link = link.replace("gdlink.dev", "new6.gdflix.dad")
    elif 'vifix.site' in link:
        link = link.replace("vifix.site/gdflix", "new6.gdflix.dad/file")

    headers = {
        'User-Agent': 'Mozilla/5.0',
        'Accept': 'text/html',
        'Referer': 'https://new5.gdflix.dad/'
    }
    ...
    raw_links = [
        a['href'] if a['href'].startswith('http') else f"https://new6.gdflix.dad{a['href']}"
    ...
    # Updated domain list
    allowed_domains = [
        'pixeldrain.net', 'r2.dev', 'gpdl', 'technorozen', 'gofile.io',
        'rapidgator.net', '1fichier.com', 'oneclick-dl.pages.dev',
        'europe.xyzcdn.cloud', 'new7.gdflix.dad', 'filesgram.site'
    ]

    allowed_exts = ['.mkv', '.mp4', '.zip', '.rar', '.7z', '.exe']
    grouped_links = {}

    def group_and_add(url):
        domain = urlparse(url).netloc
        if 'pixeldrain' in domain:
            group = '📁 PixelDrain'
        elif 'r2.dev' in domain:
            group = '🗂️ R2'
        elif 'gpdl' in domain or 'technorozen' in domain:
            group = '📦 GPDL'
            ...
            group = '🐟 1Fichier'
        elif 'busycdn' in domain or 'oneclick-dl.pages.dev' in domain:
            group = '🔍 Google Drive (CDN)'
        elif 'europe.xyzcdn.cloud' in domain or 'new7.gdflix.dad' in domain:
            group = '🧩 10GBPS Download'
        elif 'filesgram.site' in domain:
            group = '📨 Telegram'
        else:
            group = f'🔗 {domain}'
        grouped_links.setdefault(group, []).append(url)

    for url in raw_links:
        if 'goflix.sbs' in url:
            try:
                goflix_res = requests.get(url, headers=headers, timeout=10)
                if goflix_res.status_code != 200:
                    continue
                goflix_soup = B(goflix_res.text, 'html.parser')
                nested_links = [
                    a['href'] for a in goflix_soup.find_all('a', href=True)
                    if a['href'].startswith('http')
                ]
                for nlink in nested_links:
                    domain = urlparse(nlink).netloc
                    if 'busycdn' in domain or any(d in nlink for d in allowed_domains) or any(nlink.lower().endswith(ext) for ext in allowed_exts):
                        group_and_add(nlink)
            except RequestException:
                continue
        else:
            domain = urlparse(url).netloc
            if 'busycdn' in domain or any(d in url for d in allowed_domains) or any(url.lower().endswith(ext) for ext in allowed_exts):
                group_and_add(url)
    ...
    for group, urls in grouped_links.items():
        message += f"\n<b>{group}</b>:\n"
        for url in urls:
...
def hubcloud(link):    
    headers = {
        'User-Agent': 'Mozilla/5.0',
        'Accept': 'text/html',
        'Referer': 'https://hubcloud.bz/'
    }

    # Normalize domain to hubcloud.bz
    parsed_url = urlparse(link)
    if "hubcloud" in parsed_url.netloc:
        # Replace any hubcloud domain with hubcloud.bz
        new_netloc = "hubcloud.bz"
        link = urlunparse(parsed_url._replace(netloc=new_netloc))

    # Step 1: Get the first page
    response = requests.get(link, headers=headers)
    if response.status_code != 200:
        return f"❌ Error: HTTP {response.status_code} on first page"

    soup = B(response.text, 'html.parser')

    # Step 2: Find first download button/link
    first_link_tag = next(
        (a for a in soup.find_all('a', class_=lambda c: c and 'btn' in c) if a.get('href')),
        None
    )
    if not first_link_tag:
        return "❌ No download button found on the first page"

    intermediate_link = first_link_tag['href']
    if not intermediate_link.startswith("http"):
        intermediate_link = f"{parsed_url.scheme}://{parsed_url.netloc}{intermediate_link}"

    # Step 3: Visit the intermediate page
    second_response = requests.get(intermediate_link, headers=headers)
    if second_response.status_code != 200:
        return f"❌ Error: HTTP {second_response.status_code} on second page"

    second_soup = B(second_response.text, 'html.parser')
    all_links = [
        a['href'] for a in second_soup.find_all('a', href=True)
        if a['href'].startswith('http')
    ]

    # Step 4: Filter and group links
    allowed_domains = ['pixeldrain.net', 'r2.dev', 'gpdl', 'technorozen']
    allowed_exts = ['.mkv', '.mp4', '.zip', '.rar', '.7z', '.exe']

    grouped_links = {}
    for url in all_links:
        if any(domain in url for domain in allowed_domains) or any(url.lower().endswith(ext) for ext in allowed_exts):
            domain = urlparse(url).netloc
...
def qiwi(url):
    """Generates spyderrock-style direct link using real filename from Qiwi.gg page"""
    with create_scraper() as session:
        try:
        ...
        # Try multiple XPath queries to extract the filename
        possible_xpaths = [
            '//h2/text()',
            '//strong/text()',
            '//div[contains(@class, "file")]/text()',
            '//div[contains(@class, "card")]/div/text()',
        ]
        ...
        for xpath in possible_xpaths:
            ...
            if results:
                for line in results:
                    if '.' in line and any(ext in line.lower() for ext in ['.zip', '.rar', '.7z', '.exe']):
                        filename = line.strip()
                        break
            if filename:
                break
        ...
        if not filename:
            raise DirectDownloadLinkException('ERROR: Filename not found on page!')

        ext = filename.split('.')[-1]

        # Extract file ID and slug from URL
        file_segment = url.rstrip('/').split('/')[-1]
        file_id, slug = file_segment.split('-', 1)

        return f'https://spyderrock.com/{file_id}-{slug}.{ext}'
...
def tamilblasters(link):
    ...
    def find_topic_links(soup):
        return [
            a['href'] for a in soup.find_all('a', href=True)
            if re.match(r'https://www\.1tamilblasters\.moi/index\.php\?/forums/topic/\d+', a['href'])
        ]

    def find_download_links(soup):
        magnets = [a['href'] for a in soup.find_all('a', href=True) if re.search(r'magnet:.*|.*\.torrent', a['href'])]
        attachments = [
            a['href'] for a in soup.find_all('a', href=True)
            if re.match(r'https://www\.1tamilblasters\.moi/applications/core/interface/file/attachment\.php\?id=\d+', a['href'])
        ]
        return magnets, attachments

    main_page = get_soup(base_url)
    if not main_page:
        return "❌ Failed to fetch TamilBlasters homepage."

    topic_links = find_topic_links(main_page)
    if not topic_links:
        return "❌ No topics found on TamilBlasters."

    result = "<b>📥 TamilBlasters Topics:</b>\n"

    for topic in topic_links[:5]:  # Limit to 5 for brevity
        topic_soup = get_soup(topic)
        if not topic_soup:
            continue

        title_tag = topic_soup.find('title')
        title = title_tag.text.strip() if title_tag else "Untitled"

        result += f"\n<b>🔗 {title}</b>\n<code>{topic}</code>\n"

        magnets, attachments = find_download_links(topic_soup)

        for link in magnets:
            result += f"🔗 <code>{link}</code>\n"

        for att in attachments:
            result += f"📎 <code>{att}</code>\n"

        time.sleep(1)  # Optional delay to avoid spamming server

    return result if result.strip() else "❌ No download links found in topics."
...
def terabox(url):
     try:
         ...
         final_url = f"https://dzoneteraboxdownloader.ansh8012sxs.workers.dev/TheBongPirate?url={encoded_url}"
         return final_url
     except Exception as e:
