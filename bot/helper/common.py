...
class TaskConfig:
    ...
    async def editMetadata(self, path: str, gid: str):
        ...
        # Parse metadata string: author|title|subtitle|audio
        meta_parts = [x.strip() for x in metadata.split('|')]
        meta_keys = ['author', 'title', 'subtitle', 'audio']
        meta_dict = dict(zip(meta_keys, meta_parts))

        def build_metadata_args():
            args = []
            if len(meta_parts) == 1:
                # Old behavior: set all as title
                args.extend(['-metadata', f'title={meta_parts[0]}'])
                args.extend(['-metadata:s:v', f'title={meta_parts[0]}'])
                args.extend(['-metadata:s:a', f'title={meta_parts[0]}'])
                args.extend(['-metadata:s:s', f'title={meta_parts[0]}'])
            else:
                if meta_dict.get('title'):
                    args.extend(['-metadata', f'title={meta_dict["title"]}'])
                if meta_dict.get('author'):
                    args.extend(['-metadata', f'artist={meta_dict["author"]}'])
                if meta_dict.get('subtitle'):
                    args.extend(['-metadata:s:s', f'title={meta_dict["subtitle"]}'])
                    args.extend(['-metadata', f'subtitle={meta_dict["subtitle"]}'])  # for compatibility
                if meta_dict.get('audio'):
                    args.extend(['-metadata:s:a', f'title={meta_dict["audio"]}'])
            return args
        ...
        async def _run(base_dir: str, video_file: str, outfile: str, clean_metadata: bool=False):
            if clean_metadata:
                ...
                cmd += ['-map', '0:v:0?', '-map', '0:a:?', '-map', '0:s:?', '-c:v', 'copy', '-c:a', 'copy', '-c:s', 'copy',  outfile, '-y']
