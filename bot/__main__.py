...
from bot.modules import (authorize, bot_settings, clone, exec, gd_count, gd_delete, multi_search, cancel_task, mirror_leech, speed_test, status, torrent_search, torrent_select, fast_download, resume_task,
                         user_settings, ytdlp, shell, rss, wayback, hash, bypass, scrapper, purge, broadcase, info, misc_tools, backup, join_chat, video_tools, media_info, ddls, save_message, telegraph_uploader)

from bot.modules.telegraph_uploader import upload_to_cloud_function
from bot.helper.gk import mr_funct
...
async def main():
    ...
    bot.add_handler(MessageHandler(mr_funct, filters=command(BotCommands.SwapCommand)))
    bot.add_handler(MessageHandler(upload_to_cloud_function, filters=command(BotCommands.Telegraph)))
